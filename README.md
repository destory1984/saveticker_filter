...
- **수집 멈춤 알림**: 확장의 감시견이 1분마다 판별기에 소식(`/ping`)을 보낸다. 5분 넘게 소식이 없거나, 뉴스 탭이 닫혔거나,
  ...
  다시 되면 또 한 번 알린다.
- **목록 길이**: 처음에는 최근 뉴스 50개만 싣는다 (묶음 안의 뉴스도 센다). 맨 아래 **더 보기** 로 50개씩 늘린다.
...
- **시장 반응**: 판별한 모든 뉴스에 30분 뒤 나스닥(QQQ)·반도체(SOXX)·미 10년물 금리(^TNX, bp)·유가(USO)
  움직임을 붙인다. 종목이 없는 금리·유가·관세 뉴스도 시장이 어떻게 받았는지 보인다 (`시장 나스닥 +0.3% · 10년물 -1bp …`).
  나스닥·반도체 ±0.5%, 10년물 ±3bp, 유가 ±1% 를 넘으면 노란색. 점수가 낮은 뉴스에도 적는 것은 성적표에서
  "점수가 높을수록 시장이 더 움직였나" 를 견주려고다.
- **장 열기 전 브리핑** (`/briefing`): 평일 22:00(`briefing_at`)에 지난 12시간의 7점 이상 뉴스를 사건별로 묶어
  점수 높은 사건 다섯 가지를 읽고 토스트로 띄운다. PC 가 그때 잠들어 있었으면 깬 뒤 2시간 안에 한다.
  미국 서머타임이 끝나면(11월) 장이 한 시간 늦게 열리니 `"23:00"` 으로 바꾼다. 페이지의 버튼으로 바로 만들 수도 있다.
